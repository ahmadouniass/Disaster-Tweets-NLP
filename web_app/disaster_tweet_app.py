...
Streamlit wrapper → injecte l'app HTML complète via st.components.v1.html()
Le design est 100% contrôlé, aucune limitation Streamlit.
"""
...
<script>
const API_URL = 'https://your-app.onrender.com/predict'; // ← ton endpoint Render ici
...
async function analyze(){
  ...
  try {
    ...
    const demo = simulateAPI(text);
    renderResult(demo.prediction, demo.confidence);
    toast('⚡ Demo mode — connecte ton API Render dans le code.');
  } finally {
    btn.disabled = false;
    ico.textContent = '🔍';
